...
- [x] `0008_assistant_match_fn.sql` (cosine-similarity retrieval RPC; apply after 0007)

## Pure logic (`lib/`, unit-tested — 151/151 passing)
...
- [x] `assistant.js` — structural diagnostic guard + suggested questions (§18.4)
- [x] `assistantRag.js` — embed→retrieve→cite pipeline, injectable fetch/rpc (§18.2);
      pure logic tested with fakes, network gated on a model key
...
- [x] `POST /api/assistant` — diagnostic guard + grounded RAG path wired (declines honestly
      without a key; live-tested for the guard + not-configured paths)
...
- **151 unit tests pass** (`node --test test/*.test.mjs`) across all 13 logic modules.
...
1. **Apply migrations in the SQL editor, in order** (DDL can't be automated here):
   `0002` → `0003` → `0004` → `0005` → `0006` → `0007` → `0008`. This turns the 5 DEGRADED routes
...
4. **Ingest the assistant corpus** (after 0007/0008 + a model key): run
   `node scripts/ingest-corpus.mjs`. It embeds the vetted Learn sources into `assistant_corpus`;
   once rows exist, `/api/assistant` retrieves + cites them. Defaults to OpenAI
   text-embedding-3-small (1536-d); override with `ASSISTANT_EMBED_URL/_MODEL` and the answer
   model with `ASSISTANT_CHAT_URL/_MODEL`. `UPSTASH_REDIS_*` is present for the future
   per-household rate-limit (§37.2), not yet wired.

## Flagged gaps (need a key, a data source, or human work — built as far as possible)
- **Assistant grounded answers**: structural diagnostic guard, honest not-configured response, AND
  the RAG pipeline (embed → retrieve `assistant_corpus` → cited answer, `lib/assistantRag.js`) +
  the corpus ingestion script (`scripts/ingest-corpus.mjs`) are all now BUILT. The pipeline's
  decision logic is unit-tested with injected fakes; the network path is gated on a model key and
  was not run against a paid model (per instructions). To activate: apply 0007/0008, add a key,
  run the ingest script. Per-household rate-limiting (§37.2) is the remaining TODO (Upstash Redis
  creds are already in `.env.local`).
